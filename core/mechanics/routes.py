...
async def list_mechanics(
    ...
    query = db.query(MechanicModel).options(
...
async def get_mechanic_public(
    mechanic_id: int,
    db: Session = Depends(get_db),
):
    mechanic = db.query(MechanicModel).options(
        joinedload(MechanicModel.user),
        joinedload(MechanicModel.specializations),
        joinedload(MechanicModel.reviews),
    ).filter_by(id=mechanic_id, is_verified=True).first()
...
async def update_mechanic_profile(
    ...
    if request.workshop_name is not None:
        mechanic.workshop_name = request.workshop_name
    if request.address is not None:
        mechanic.address = request.address
    if request.national_id is not None:
        mechanic.national_id = request.national_id
    ...
    # ─── reload با joinedload ───────────────────────────────────────────────
    mechanic = db.query(MechanicModel).options(
        joinedload(MechanicModel.user),
        joinedload(MechanicModel.specializations),
        joinedload(MechanicModel.reviews),
    ).filter_by(id=mechanic.id).first()
