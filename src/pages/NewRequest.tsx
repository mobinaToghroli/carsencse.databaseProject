...
export default function NewRequest() {
  ...
  // ─── فایل‌ها — File object نگه می‌داریم (نه blob URL) ─────────────────────
        ...
        <div className="flex min-h-[80vh] items-center justify-center p-8">
          <div className="max-w-md rounded-2xl border border-[#06B6D4]/20 bg-[#1E293B] p-8 text-center">
            ...
            <p className="mt-1 text-sm text-[#94A3B8]">
              شماره درخواست:{' '}
              <span className="font-mono text-base font-bold text-[#06B6D4]">#{submitted}</span>
            </p>
            <div className="mt-6 flex gap-3">
              <button onClick={() => navigate('/user/history')}
                className="flex-1 rounded-lg bg-gradient-to-r from-[#3B82F6] to-[#06B6D4] px-4 py-2.5 text-sm font-semibold text-white">
                مشاهده تاریخچه
              </button>
              <button onClick={() => {
                setSubmitted(null);
                setFormData({ issueDescription: '', category: 'other', priority: 'normal' });
                setImageFiles([]); setImagePreviews([]);
                setAudioFiles([]); setAudioUrls([]);
                setSelectedVehicleId(vehicles.length === 1 ? vehicles[0].id : null);
              }} className="flex-1 rounded-lg border border-[#1E293B] bg-[#0F172A] px-4 py-2.5 text-sm font-semibold text-[#94A3B8]">
                ثبت درخواست جدید
              </button>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-[#94A3B8]">
                انتخاب خودرو <span className="text-[#EF4444]">*</span>
              </label>
              {loadingVehicles ? (
                ...
                <div className="rounded-lg border border-dashed border-[#3B82F6]/30 bg-[#0F172A] p-4 text-center">
                  ...
                  <Link to="/user/vehicles" className="mt-2 inline-block text-sm text-[#3B82F6] hover:underline">
                    افزودن خودرو
                  </Link>
                </div>
              ) : (
                  ...
                  {vehicles.map((v) => (
                    <button key={v.id} type="button" onClick={() => setSelectedVehicleId(v.id)}
                      ...
                      <div className="min-w-0">
                        ...
                        {v.plate && <p className="font-mono text-xs text-[#94A3B8]">{v.plate}</p>}
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[{ value: 'normal', label: 'عادی' }, { value: 'urgent', label: 'فوری' }, { value: 'emergency', label: 'اضطراری' }].map((p) => (
                  <button key={p.value} type="button" onClick={() => setFormData({ ...formData, priority: p.value as any })}
                    className={`rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors ${formData.priority === p.value
                      ? p.value === 'emergency' ? 'border-[#EF4444] bg-[#EF4444]/10 text-[#EF4444]'
                        : p.value === 'urgent' ? 'border-amber-500 bg-amber-500/10 text-amber-400'
                        : 'border-[#94A3B8] bg-[#94A3B8]/10 text-[#94A3B8]'
                      : 'border-[#0F172A] bg-[#0F172A] text-[#94A3B8]'}`}>
                    {p.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[#94A3B8]">
                شرح مشکل <span className="text-[#EF4444]">*</span>
              </label>
              <textarea value={formData.issueDescription}
                ...
                required />
              <p className="mt-1 text-xs text-[#94A3B8]">{formData.issueDescription.length} کاراکتر (حداقل ۱۰)</p>
            </div>

            {/* ─── تصاویر ─── */}
            <div>
              <label className="mb-2 block text-sm font-medium text-[#94A3B8]">
                تصاویر (حداکثر ۵ عدد، هر فایل تا ۵MB)
              </label>
                ...
                {imagePreviews.map((img, i) => (
                  <div key={i} className="relative h-20 w-20 overflow-hidden rounded-lg border border-[#1E293B]">
                    ...
                    <button type="button" onClick={() => removeImage(i)}
                      className="absolute right-0 top-0 flex h-5 w-5 items-center justify-center rounded-full bg-[#EF4444] text-white">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
                {imageFiles.length < 5 && (
                  <label className="flex h-20 w-20 cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-[#3B82F6]/30 bg-[#0F172A] hover:border-[#3B82F6]/60">
                    <div className="text-center">
                      <ImagePlus className="mx-auto h-6 w-6 text-[#3B82F6]" />
                      <span className="mt-1 block text-[10px] text-[#94A3B8]">افزودن</span>
                    </div>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-[#94A3B8]">
                صدا (حداکثر ۲ فایل، هر فایل تا ۱۰MB)
              </label>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <label className={`flex cursor-pointer items-center gap-2 rounded-xl border border-[#1E293B] bg-[#0F172A] px-4 py-2.5 text-sm font-semibold text-[#94A3B8] hover:text-white ${audioFiles.length >= 2 ? 'cursor-not-allowed opacity-40' : ''}`}>
                ...
                <div className="mt-3 space-y-2">
                  {audioUrls.map((url, i) => (
                    <div key={i} className="flex items-center gap-2 rounded-lg bg-[#0F172A] px-3 py-2">
                      ...
                      <button type="button" onClick={() => removeAudio(i)} className="flex-shrink-0 text-[#EF4444]">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  ))}
            ...
            <button type="submit" disabled={submitting || !selectedVehicleId || vehicles.length === 0}
              ...
              {submitting ? (
                ...
                <>ثبت درخواست <ArrowRight className="h-4 w-4" /></>
              )}
            </button>
          </form>
        </div>
