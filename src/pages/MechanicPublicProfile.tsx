...
export default function MechanicPublicProfile() {
  ...
  const requestForMechanic = () => {
    ...
    if (currentUser.role !== 'user') { alert('برای ثبت درخواست با حساب کاربری وارد شوید'); return; }
          ...
          <div className="mx-auto max-w-[1100px] px-8">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="rounded-3xl border border-[#1E293B] bg-[#1E293B] p-7 text-center">
                <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-3xl bg-gradient-to-br from-[#3B82F6] to-[#06B6D4] text-5xl font-black text-white">{mechanic.full_name.charAt(0)}</div>
                <h1 className="mt-5 text-2xl font-black text-[#F8FAFC]">{mechanic.full_name}</h1>
                {mechanic.specializations.length > 0 && <p className="mt-2 text-sm text-[#3B82F6]">{mechanic.specializations.map((s) => s.name).join(' · ')}</p>}
                ...
                <button onClick={requestForMechanic} className="mt-7 w-full rounded-2xl bg-gradient-to-r from-[#3B82F6] to-[#06B6D4] px-6 py-3.5 text-sm font-black text-white shadow-lg">ثبت درخواست</button>
              </div>
              ...
              <div className="rounded-3xl border border-[#1E293B] bg-[#1E293B] p-7 lg:col-span-2">
                ...
                <p className="mt-3 text-sm leading-7 text-[#94A3B8]">{mechanic.bio || 'توضیحی ثبت نشده است.'}</p>
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {mechanic.years_of_experience > 0 && <div className="rounded-xl bg-[#0F172A] p-3"><p className="text-xs text-[#94A3B8]">تجربه</p><p className="text-sm font-bold text-white">{mechanic.years_of_experience} سال</p></div>}
                  {mechanic.city && <div className="rounded-xl bg-[#0F172A] p-3"><p className="text-xs text-[#94A3B8]">شهر</p><p className="text-sm font-bold text-white">{mechanic.city}</p></div>}
                  <div className="rounded-xl bg-[#0F172A] p-3"><p className="text-xs text-[#94A3B8]">تعمیرات تکمیل شده</p><p className="text-sm font-bold text-white">{mechanic.total_completed}</p></div>
                </div>
                {reviews.length > 0 && (
                  <div className="mt-6">
                    ...
                    <div className="space-y-3">
                      {reviews.slice(0, 3).map((r) => (
                        <div key={r.id} className="rounded-xl bg-[#0F172A] p-4">
                          <div className="flex items-center gap-1 text-amber-400 mb-1">{[...Array(r.rating)].map((_, i) => <Star key={i} className="h-3.5 w-3.5 fill-current" />)}</div>
      ...
      {showLoginPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowLoginPrompt(false)}>
          <div className="w-full max-w-sm rounded-2xl border border-[#1E293B] bg-[#1E293B] p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#3B82F6]/10"><Shield className="h-6 w-6 text-[#3B82F6]" /></div><button onClick={() => setShowLoginPrompt(false)} className="text-[#94A3B8]"><X className="h-5 w-5" /></button></div>
